        ...
        if (dousedPlayers.size() >= ArsonistConstants.getRequiredDousedCount(alivePlayers.size())) {
            ...
            if (playersLeft == 1) {
                VictoryApi.endGameWithCustomVictory(
                        serverWorld,
                        CustomVictory.of(NoellesRoleRegistry.ARSONIST.identifier(), NoellesRoleRegistry.ARSONIST.color(), List.of(player))
                );
            }
        } else {
            player.playSoundToPlayer(SoundEvents.BLOCK_FIRE_EXTINGUISH, SoundCategory.PLAYERS, 1.0F, 1.0F);
            GameFunctions.killPlayer(player, true, player, NoellesDeathReasons.ARSONIST_FAILED_IGNITE_DEATH_REASON);
        }
