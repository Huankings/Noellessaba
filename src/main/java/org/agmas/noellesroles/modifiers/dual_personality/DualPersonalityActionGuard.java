...
public final class DualPersonalityActionGuard {
    ...
    public static boolean isDormant(ServerPlayerEntity player) {
        /*
         * 这里只认 ACTIVE 局内阶段。
         ...
         */
        return player != null
                && DualPersonalityManager.isActiveRound(player.getWorld())
                && DualPersonalityComponent.KEY.get(player.getWorld()).isDormant(player.getUuid());
    }

    public static void maintainDormantLock(ServerPlayerEntity player) {
        if (!isDormant(player)) {
            return;
        }
    ...
    private static void applyNoellesAbilityCooldown(ServerPlayerEntity player) {
        // 迁移到 NoellesRoles 后，自己的通用能力冷却组件可以直接访问，保证所有 Noelles 能力包都会被休眠人格封锁。
    ...
    private static void applyItemCooldowns(ServerPlayerEntity player) {
        /*
         * 物品冷却比拦每一个使用包更兼容。
        ...
        addRegisteredItem(items, "noellesroles", "bayonet");
        addRegisteredItem(items, "noellesroles", "sniper_rifle");
        addRegisteredItem(items, "noellesroles", "hunting_knife");
        addRegisteredItem(items, "noellesroles", "blowgun");
        addRegisteredItem(items, "noellesroles", "knockout_drug");
        addRegisteredItem(items, "noellesroles", "poison_injector");
        addRegisteredItem(items, "noellesroles", "pan");
        addRegisteredItem(items, "noellesroles", "tape");

        // 其他扩展物品只通过注册表软取，避免把未要求搬运的源码做成硬依赖。
        ...
        for (Item item : items) {
            if (item != null) {
                player.getItemCooldownManager().set(item, DORMANT_LOCK_TICKS);
            }
        }
    }
    ...
    private static void applyReflectedAbilityCooldown(ServerPlayerEntity player, String componentClassName) {
        try {
            /*
            ...
            if (REPORTED_REFLECTION_FAILURES.add(componentClassName)) {
                NoellesRolesCore.LOGGER.warn("双重人格休眠人格跨模组技能封锁桥接失败：{}", componentClassName, exception);
            }
        }
    }
