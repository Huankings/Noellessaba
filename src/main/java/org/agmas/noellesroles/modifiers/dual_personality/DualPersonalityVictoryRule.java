...
public final class DualPersonalityVictoryRule {
    ...
    public static void init() {
        VictoryApi.registerRule(NoellesRolesCore.id("victory/dual_personality"), PRIORITY, context -> {
            ...
            List<ServerPlayerEntity> remainingDualPersonalities = remainingPlayers.stream()
                    .filter(player -> modifierComponent.isModifier(player, NoellesModifierRegistry.DUAL_PERSONALITY))
                    .toList();
            if (remainingDualPersonalities.isEmpty()) {
                return VictoryApi.VictoryResult.pass();
            }
            List<UUID> allDualPersonalityUuids = modifierComponent.getAllWithModifier(NoellesModifierRegistry.DUAL_PERSONALITY);
            ...
            if (remainingPlayers.size() == remainingDualPersonalities.size()) {
                return NoellesRolesVictoryUtil.customWinUuids(
                        NoellesModifierRegistry.DUAL_PERSONALITY.identifier(),
                        NoellesModifierRegistry.DUAL_PERSONALITY.color(),
            ...
            if (context.vanillaWinStatus() == GameFunctions.WinStatus.KILLERS) {
                if (DualPersonalityConstants.WIN_WITH_KILLERS && anyDualPersonalityKiller) {
            ...
            if (context.vanillaWinStatus() == GameFunctions.WinStatus.PASSENGERS) {
                if (DualPersonalityConstants.WIN_WITH_CIVILIANS) {
                    return VictoryApi.VictoryResult.vanillaWin(
    ...
    private static List<UUID> collectIndependentWinnerUuids(
            List<ServerPlayerEntity> remainingDualPersonalities,
        ...
        for (ServerPlayerEntity player : remainingDualPersonalities) {
