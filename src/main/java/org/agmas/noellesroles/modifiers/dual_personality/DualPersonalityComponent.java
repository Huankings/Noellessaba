...
public class DualPersonalityComponent implements AutoSyncedComponent {
    // CCA 的组件键。注册在世界组件上后，服务端和客户端都可以通过 KEY.get(world) 读取当前对局状态。
    public static final ComponentKey<DualPersonalityComponent> KEY = ComponentRegistry.getOrCreate(
            Identifier.of(NoellesRolesCore.MOD_ID, "dual_personality"),
            DualPersonalityComponent.class
    );
    ...
    public DualPersonalityComponent(World world) {
        this.world = world;
    }
    ...
    public void readFromNbt(NbtCompound tag, RegistryWrapper.WrapperLookup registryLookup) {
         ...
         */
        for (NbtElement element : tag.getList(PAIRS_KEY, NbtElement.COMPOUND_TYPE)) {
            if (!(element instanceof NbtCompound pairTag) || !pairTag.containsUuid(MAIN_KEY) || !pairTag.containsUuid(SUB_KEY)) {
                continue;
            }

            UUID main = pairTag.getUuid(MAIN_KEY);
            UUID sub = pairTag.getUuid(SUB_KEY);
            PairState state = new PairState(main, sub, pairTag.getBoolean(FORCED_KEY));
            if (pairTag.containsUuid(ACTIVE_KEY)) {
                state.active = pairTag.getUuid(ACTIVE_KEY);
            }
            if (pairTag.containsUuid(DORMANT_KEY)) {
                state.dormant = pairTag.getUuid(DORMANT_KEY);
            }
    ...
    public void writeToNbt(NbtCompound tag, RegistryWrapper.WrapperLookup registryLookup) {
        NbtList pairList = new NbtList();
        for (PairState pair : this.pairs) {
            // 这里保存的是完整运行态，不只是配对关系；掉线暂停、倒计时、双活剩余时间都必须一起落盘。
            NbtCompound pairTag = new NbtCompound();
            pairTag.putUuid(MAIN_KEY, pair.main);
            pairTag.putUuid(SUB_KEY, pair.sub);
            pairTag.putUuid(ACTIVE_KEY, pair.active);
            pairTag.putUuid(DORMANT_KEY, pair.dormant);
    ...
    public static class PairState {
        ...
        public int switchTicks = DualPersonalityConstants.SWITCH_INTERVAL_TICKS;
        ...
        public int initialMessageDelayTicks = DualPersonalityConstants.INITIAL_ROLE_MESSAGE_DELAY_TICKS;
        // 开局短时间内强制补发附身相机包，避免客户端错过第一次 setCameraEntity 后只看到自己被 TP 到主人格身边。
        public transient int initialCameraSyncTicks = DualPersonalityConstants.INITIAL_CAMERA_SYNC_TICKS;
