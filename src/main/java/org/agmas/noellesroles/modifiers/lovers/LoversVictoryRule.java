 ...
 * 恋人词条的独立胜利 / 共胜 / 保活规则。
 *
 * <p>拆到 modifiers.lovers 包里，是为了让“恋人自己的胜利规则”跟配对组件、
...
public final class LoversVictoryRule {
    ...
    public static void init() {
        VictoryApi.registerRule(org.agmas.noellesroles.registry.NoellesRolesCore.id("victory/lovers"), PRIORITY, context -> {
             ...
             */
            List<ServerPlayerEntity> remainingPlayers = context.alivePlayers();
            List<ServerPlayerEntity> remainingLovers = remainingPlayers.stream()
                    .filter(player -> modifierComponent.isModifier(player, NoellesModifierRegistry.LOVERS))
                    .toList();
             ...
             */
            List<UUID> remainingLoverUuids = remainingLovers.stream().map(ServerPlayerEntity::getUuid).toList();
            List<UUID> allLoverUuids = modifierComponent.getAllWithModifier(NoellesModifierRegistry.LOVERS);
            ...
            if (remainingPlayers.size() == remainingLovers.size()) {
                return NoellesRolesVictoryUtil.customWinUuids(
                        NoellesModifierRegistry.LOVERS.identifier(),
                        NoellesModifierRegistry.LOVERS.color(),
             ...
             * 3. 配置允许的杀手恋人共胜条件成立。
             */
            if (!LoversConstants.WIN_WITH_CIVILIANS
    ...
    private static VictoryApi.VictoryResult handleHeartbreakDeaths(
            List<ServerPlayerEntity> remainingLovers,
         ...
         */
        for (ServerPlayerEntity lover : remainingLovers) {
            ...
            if (partnerUuid == null || !remainingLoverUuids.contains(partnerUuid)) {
                ...
                GameFunctions.killPlayer(lover, true, null, NoellesDeathReasons.BROKEN_HEART_DEATH_REASON, extraDeathData);
                return VictoryApi.VictoryResult.keepRunning();
            }
        }
        return VictoryApi.VictoryResult.pass();
    ...
    private static VictoryApi.VictoryResult tryKillerCoWin(
            List<ServerPlayerEntity> remainingPlayers,
            List<ServerPlayerEntity> remainingLovers,
            List<UUID> remainingLoverUuids,
            LoversPairComponent pairComponent,
         ...
         *
         * 因此这里主动检查 NoellesRoles 自己的条件，满足时直接返回 vanillaWin(KILLERS, extraWinners)。
         */
        for (ServerPlayerEntity player : remainingLovers) {
            ServerPlayerEntity partner = getLivingPartner(player, pairComponent, remainingLoverUuids, remainingLovers);
            if (partner == null) {
                continue;
            }
            /*
             ...
             */
            List<ServerPlayerEntity> remainingNonInnocent = remainingPlayers.stream()
    ...
    private static ServerPlayerEntity getLivingPartner(
