...
public final class WinderInventoryButtons {
    private WinderInventoryButtons() {
    }

    public static void register() {
        NoellesInventoryButtonSupport.registerLimited("winder", WinderInventoryButtons::create);
    }

    private static @Nullable InventoryButtonExtension create(@NotNull InventoryButtonContext context) {
        return NoellesInventoryButtonSupport.isRole(context.requirePlayer(), NoellesRoleRegistry.WINDER) ? new Extension() : null;
    }

    private static final class Extension extends NoellesInventoryButtonSupport.PagedExtension<WinderPlayerWidget> {
        private Extension() {
            super("winder");
        }
        ...
        protected void populate(@NotNull InventoryButtonContext context, @NotNull LimitedInventoryScreen screen, @NotNull ClientPlayerEntity player) {
            ...
            for (UUID targetUuid : NoellesInventoryButtonSupport.onlineUuids(player)) {
                this.buttons.addWidget(context, new WinderPlayerWidget(screen, 0, y, targetUuid, NoellesInventoryButtonSupport.entry(player, targetUuid)));
            }
        }
    }
}
