...
public class GoddessPlayerWidget extends ButtonWidget {
    private final LimitedInventoryScreen screen;
    private final UUID targetUuid;
    private final PlayerListEntry entry;

    public GoddessPlayerWidget(LimitedInventoryScreen screen, int x, int y, UUID targetUuid, PlayerListEntry entry) {
        super(x, y, 16, 16, Text.literal(entry.getProfile().getName()),
                button -> {
                    ...
                    if (ability.cooldown == 0) {
                        ClientPlayNetworking.send(new GoddessC2SPacket(targetUuid));
                    }
                },
